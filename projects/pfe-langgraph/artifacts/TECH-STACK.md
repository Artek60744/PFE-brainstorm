...
This document **centralizes the technology stack** for the PFE LangGraph RPAE agent, eliminating redundant definitions scattered across PROJECT.md, project.yaml, decisions, and artifacts.

**CHANGE v2.0**: OpenClaw replaced by LangGraph + APScheduler (ADR-017)
...
│  Memory: SqliteSaver / PostgresSaver                 │
└──────────────────────────────────────────────────────┘
        ↓            ↓            ↓            ↓
...
|-----------|-----------|---------|---------|-------|
| **Agent Runtime** | LangGraph | Latest | Read-Plan-Approve-Execute orchestration | StateGraph + checkpointer |
| **Heartbeat** | APScheduler | 3.10+ | Continuous pipeline monitoring | Cron-like scheduling |
| **Memory** | LangGraph SqliteSaver | Built-in | Persistent state per pipeline | Upgraded to PostgresSaver V2 |
| **Integration Protocol** | MCP (Model Context Protocol) | 1.0+ | Safe system integrations | Python SDK |
| **Language** | Python | 3.10+ | Dashboard + diagnostic logic | FastAPI web framework |

**References**:
- ADR-017: LangGraph replaces OpenClaw
...
**ADR**: ADR-002, ADR-009, ADR-018 (webhook + polling fallback), ADR-018 (webhook + polling fallback)
...
**ADR**: ADR-019 (polling adaptatif 15s-10min)
...
**ADR**: ADR-010 (FastAPI + HTMX), ADR-020 (HTMX adaptatif + BroadcastChannel), ADR-020 (HTMX adaptatif + BroadcastChannel)
...
| LangGraph | $0 | $0 | MIT license, open-source |
| APScheduler | $0 | $0 | MIT license, open-source |
...
```
MVP (S1-S15): LangGraph + SQLite + APScheduler, Python 3.10, FastAPI 0.100
...
## Part 11: LangGraph Architecture Details
...
# Define the RPAE workflow
workflow = StateGraph(RPAEState)

workflow.add_node("read", read_incident)
workflow.add_node("plan", generate_plan)
workflow.add_node("approve", wait_approval)
workflow.add_node("execute", execute_action)

workflow.set_entry_point("read")
workflow.add_edge("read", "plan")
...
with SqliteSaver.from_conn_string("openclaw.db") as checkpointer:
    app = workflow.compile(checkpointer=checkpointer)
```

### Heartbeat with APScheduler
...
- **ADR-017**: LangGraph replaces OpenClaw
- **ADR-001**: ~~OpenClaw~~ → Superseded by ADR-017
